...
class ApplicationQueryAndStatementManager:
    # TODO УПРОСТИТЬ !!! (блок с количеством и пагинацией))
    @staticmethod
    async def get_applications(
        ...
        if application_type == "MT":
            _filters.append(Application.type == APPLICATION_TYPE_MAPPING["MT"])
    ...
    @staticmethod
    async def get_application_identifiers_by_directory_identifier(
        session: AsyncSession,
        ...
        if directory_id:
            _filters.append(Application.directory_id == directory_id)
        if directory_uuid:
            _filters.append(Application.directory_uuid == directory_uuid)
        
        query = (
            select(Application.id, Application.uuid)
            .filter(
                and_(
                    *_filters
                )
