...
class BankDetailsService:
    ...
    async def create_banks_details(
        ...
        if requester_user_privilege != PRIVILEGE_MAPPING["Admin"]:
            ...
            assert len(le_uuid_tuple) == 1, "Вы не можете добавлять реквизиты к ЮЛ других Пользователей!"
            ...
            assert le_check_access_response_object, "Вы не можете добавлять реквизиты к ЮЛ других Пользователей!"
    ...
    async def get_banks_details(
        ...
        if requester_user_privilege != PRIVILEGE_MAPPING["Admin"]:
            assert any([legal_entity_uuid, user_uuid]), "Вы не можете просмотреть Реквизиты других Пользователей!"
            if user_uuid:
                assert requester_user_uuid == user_uuid, "Вы не можете просмотреть Реквизиты других Пользователей!"
            if legal_entity_uuid:
                ...
                assert le_check_access_response_object, "Вы не можете просмотреть информацию о Реквизитах в ЮЛ других Пользователей!"
    ...
    async def update_bank_details(
        ...
        if requester_user_privilege != PRIVILEGE_MAPPING["Admin"]:
            ...
            assert bank_details, "Ошибка доступа к записи о Реквизитах!"
            assert bank_details[0].user_uuid == requester_user_uuid, "Вы не можете обновить запись о Реквизитах другого Пользователя!"
            if bank_details[0].legal_entity_uuid:
                ...
                assert le_check_access_response_object, "Вы не можете обновить информацию о Реквизитах в ЮЛ других Пользователей!"
        assert list(
            filter(
                lambda x: x != "~",
                [
                    user_uuid, from_customer, legal_entity_uuid,
        ...
        ), "Хотя бы одно поле должно быть изменено для обновления данных о Реквизитах!"
    ...
    async def delete_banks_details(
        ...
        assert bank_details_ids, "Должен быть указан хотя бы 1 ID банковских реквизитов к удалению!"
        if requester_user_privilege != PRIVILEGE_MAPPING["Admin"]:
            banks_details: List[Optional[BankDetails]] = await BankDetailsQueryAndStatementManager.get_banks_details(
                session=session,
            ...
            assert len(user_uuid_tuple) == 1 and user_uuid_tuple[0] == requester_user_uuid, "Вы не можете удалять записи о Реквизитах других Пользователей или же Реквизиты с данным id уже удалены!"
            if le_uuid_tuple:
                assert len(le_uuid_tuple) == 1, "Вы не можете удалять записи о Реквизитах других Пользователей!"
                le_check_access_response_object: Optional[Tuple[int, int, str]] = await LegalEntityQueryAndStatementManager.check_access(
                    session=session,
                ...
                assert le_check_access_response_object, "Вы не можете удалять записи о Реквизитах других Пользователей!"
